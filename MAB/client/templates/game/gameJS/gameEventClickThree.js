...
'click .three': function (event) {
    //if ( Session.get('blockClick') == 0) {//allow click 
        ...
        var REWARD = Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]}).rewards[Session.get('roundNumber') - 1][2];   //generate reward
        

        var a = Session.get('pointThree');
        var b = REWARD;
        Session.set('pointThree', a + b);

        var image = document.getElementById('armThreeGame');
        if (REWARD) {
        image.src = "win.png";
        } else {
        image.src = "fail.png";
        ...
        Games.update({ _id : Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]})._id}, { $inc : { 'box.2' : 1}}); //record the total click number that box3 got
        Session.set('boxThree', Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]}).box[2]);

        Games.update({ _id : Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]})._id}, { $set : { 'boxCurrent.2' : 1}});
        ...
        var index = Session.get('roundNumber') - 1;
        var setModifier = { $set: {} };
        setModifier.$set['choices.' + index] = [3, REWARD]; //record the click result in Games.choices : [box, point]
        ...
        Tracker.autorun(function (trackThree){
            ...
            if (clickCheckFunc() == groupSize) {
                ...
                elemOne.style.width = widthOne + '%'; //bar的长度
