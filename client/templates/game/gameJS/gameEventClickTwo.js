...
'click .two': function (event) {
    //if ( Session.get('blockClick') == 0) {//allow click 
        ...
        var REWARD = Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]}).rewards[Session.get('roundNumber') - 1][1];  //reward function  

        var a = Session.get('pointTwo');
        var b = REWARD;
        Session.set('pointTwo', a + b);

        var image = document.getElementById('armTwoGame');
        if (REWARD) {
        image.src = "win.png";
        } else {
        image.src = "fail.png";
        ...
        Games.update({ _id : Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]})._id}, { $inc : { 'box.1' : 1}}); //record the total click number that box2 got
        Session.set('boxTwo', Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]}).box[1]);

        Games.update({ _id : Games.findOne({ name : Session.get('NAME'), time : Session.get('timeArray')[0]})._id}, { $set : { 'boxCurrent.1' : 1}});
        ...
        var index = Session.get('roundNumber') - 1;
        var setModifier = { $set: {} };
        setModifier.$set['choices.' + index] = [2, REWARD]; //record the click result in Games.choices : [box, point]
        ...
        //Games.update({ _id : Games.findOne({ userId : Meteor.userId()})._id}, { $inc : { 'box.1' : 1}}); //record the total click number that box2 got
        ...
        Tracker.autorun(function (trackTwo){
            ...
            if (clickCheckFunc() == groupSize) {
                ...
                var index = Session.get('roundNumber') - 1; //cuz the roundNumber just +1, so need to -2 to get the right index 
